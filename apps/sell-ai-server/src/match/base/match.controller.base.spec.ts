...
const CREATE_INPUT = {
  ...
  customerUser: "exampleCustomerUser",
  id: "exampleId",
  supplier: "exampleSupplier",
  supplierUser: "exampleSupplierUser",
  updatedAt: new Date(),
};
const CREATE_RESULT = {
  createdAt: new Date(),
...
const service = {
  ...
  matches: () => FIND_MANY_RESULT,
  match: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("Match", () => {
  ...
  test("POST /matches existing resource", async () => {
